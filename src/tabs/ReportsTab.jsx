...
const ReportsTab = ({ data, refreshData }) => {
    ...
    const [storageSize, setStorageSize] = useState(MockBackend.getStorageSize ? MockBackend.getStorageSize() : "0.00");
    ...
    const handleFile = (e) => {
        ...
        reader.onload = (e) => {
            ...
            if (success) {
                ...
                if (MockBackend.getStorageSize) setStorageSize(MockBackend.getStorageSize());
    ...
    return (
        <div className="p-4 md:p-8 space-y-8 fade-in pb-32">
                ...
                <div className="fixed inset-0 bg-black/80 z-[100] flex items-center justify-center p-4 fade-in" onClick={() => setPreviewFile(null)}>
                    <div className="relative max-w-4xl max-h-[90vh] w-full bg-transparent" onClick={e => e.stopPropagation()}>
                        ...
                        {previewFile.type.includes('image') ? (
                            <img src={previewFile.content} className="w-full h-auto max-h-[80vh] object-contain rounded-lg shadow-2xl bg-black" alt="Preview" />
                        ) : (
            ...
            <div className="flex justify-between items-center">
                ...
                {/* Optional Storage Indicator */}
                <div className="text-xs text-slate-400 font-mono hidden md:block">
                    Storage Used: {storageSize} MB
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden border border-slate-100">
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-slate-50 text-slate-500 text-sm border-b border-slate-200">
                                <th className="px-6 py-4 font-bold cursor-pointer hover:text-blue-600 transition" onClick={() => requestSort('name')}>
                                    Document Name {sortConfig.key === 'name' && (sortConfig.direction === 'ascending' ? '▲' : '▼')}
                                </th>
                                <th className="px-6 py-4 font-bold cursor-pointer hover:text-blue-600 transition" onClick={() => requestSort('doctor')}>
                                    Doctor {sortConfig.key === 'doctor' && (sortConfig.direction === 'ascending' ? '▲' : '▼')}
                                </th>
                                <th className="px-6 py-4 font-bold cursor-pointer hover:text-blue-600 transition" onClick={() => requestSort('date')}>
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {sortedReports.length === 0 ? (
                                ...
                                sortedReports.map((report) => (
                                    <tr 
                                        key={report.id} 
                                        className="group hover:bg-slate-50/50 transition cursor-pointer"
                ...
                <div className="p-4 bg-slate-50 border-t flex justify-between items-center">
                    ...
                    <div className="flex gap-1">
                        <button className="p-1 px-2 text-xs rounded border bg-white text-slate-400 hover:bg-slate-50" disabled>Prev</button>
                        <button className="p-1 px-2 text-xs rounded border bg-blue-50 text-blue-600 font-bold border-blue-200">1</button>
                        <button className="p-1 px-2 text-xs rounded border bg-white text-slate-400 hover:bg-slate-50" disabled>Next</button>
                    </div>
                </div>
            </div>
        </div>
